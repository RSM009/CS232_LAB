...
;         }
 ...
 mov r11, 0                ; ; initialize j = 0 rax =i                                    
    outer_loop:
        cmp r11, r9            ; ; compare j with c2
        jnl end_outer_loop             ; ; if j >= c2, end outer loop
        mov rax, 0  ;                    ; ; initialize i = 0 rax ==j
        middle_loop:                       
            cmp rax, rsi                       ; ; compare j with c2
            jnl end_middle_loop            ; ; if j >= c2, end middle loop
            ; mov r14, 0                     ; ; initialize mat3[i][j] = 0
            ; mov rcx, [rcx + 0]            ; ; rcx = &mat2[0][0]
             mov r12, 0                     ; ; initialize k = 0
            inner_loop:
       
                cmp r12, r8            ; ; compare k with c1
                jnl end_inner_loop             ; ; if k >= c1, end inner loop
